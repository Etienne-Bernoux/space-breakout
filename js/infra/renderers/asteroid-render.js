...
function styleAlienCore(ctx, a, rx, ry, tp) {
  // Base métal (même gradient que styleMetal)
  tp(ctx, a.shape, rx, ry);
  const grad = ctx.createLinearGradient(-rx, -ry, rx, ry);
  grad.addColorStop(0, '#aaaabc');
  grad.addColorStop(0.3, '#888899');
  grad.addColorStop(0.5, '#777788');
  grad.addColorStop(0.7, '#555566');
  ...
  // Excroissances organiques vertes (par-dessus le métal)
  for (const v of a.veins) {
    // Veine épaisse organique
    ctx.beginPath();
    ctx.moveTo(v.x1, v.y1);
    ctx.quadraticCurveTo(v.cpx, v.cpy, v.x2, v.y2);
    ctx.strokeStyle = `rgba(30,140,50,${0.6 + pulse})`;
    ctx.lineWidth = v.width * 1.5;
    ctx.stroke();
    // Glow bioluminescent
    ctx.strokeStyle = `rgba(80,255,120,${0.2 + pulse})`;
    ctx.lineWidth = v.width * 3;
    ctx.stroke();
  }

  // Taches organiques sur les cratères (mousse alien)
  for (const c of a.craters) {
    const crx = rx * c.ox;
    const cry = ry * c.oy;
    const mossGrad = ctx.createRadialGradient(crx, cry, 0, crx, cry, c.r * 1.5);
    mossGrad.addColorStop(0, `rgba(40,180,60,${0.4 + pulse})`);
    mossGrad.addColorStop(0.6, `rgba(30,120,40,${0.2 + pulse * 0.5})`);
    mossGrad.addColorStop(1, 'rgba(0,0,0,0)');
    ctx.fillStyle = mossGrad;
    ctx.beginPath();
    ctx.arc(crx, cry, c.r * 1.5, 0, Math.PI * 2);
    ctx.fill();
  }

  // Œil central (parasite)
  const eyeR = Math.min(rx, ry) * 0.22;
  ctx.beginPath();
  ctx.arc(0, 0, eyeR * 1.4, 0, Math.PI * 2);
  ctx.fillStyle = '#0a1a0a';
  ctx.fill();
  ...
  // Contour : mix métal + vert organique
  tp(ctx, a.shape, rx, ry);
  ctx.strokeStyle = `rgba(80,255,120,${0.2 + pulse * 0.3})`;
  ctx.lineWidth = 1.5;
  ctx.stroke();
  drawRim(ctx, tp, a, rx, ry, 0.15, 0.3);
  drawDamageOverlay(ctx, tp, a, rx, ry);
}
