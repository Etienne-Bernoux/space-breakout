// --- Ship Renderer ---
// Rendu canvas du vaisseau, extrait de Ship.draw()

export function drawShip(ctx, ship) {
  const cx = ship.x + ship.width / 2;
  const { y, width, height, color } = ship;
  ...
  // Aile gauche
  const wingGrad = ctx.createLinearGradient(ship.x, y, cx, y + height);
  wingGrad.addColorStop(0, '#005580');
  wingGrad.addColorStop(1, '#003350');
  ctx.fillStyle = wingGrad;
  ctx.beginPath();
  ctx.moveTo(cx - 10, y + 2);
  ctx.lineTo(ship.x - 4, y + height + 4);
  ctx.lineTo(ship.x + 10, y + height - 1);
  ctx.lineTo(cx - 8, y + height);
  ctx.closePath();
  ctx.fill();

  // Aile droite
  const wingGrad2 = ctx.createLinearGradient(ship.x + width, y, cx, y + height);
  wingGrad2.addColorStop(0, '#005580');
  wingGrad2.addColorStop(1, '#003350');
  ctx.fillStyle = wingGrad2;
  ctx.beginPath();
  ctx.moveTo(cx + 10, y + 2);
  ctx.lineTo(ship.x + width + 4, y + height + 4);
  ctx.lineTo(ship.x + width - 10, y + height - 1);
  ctx.lineTo(cx + 8, y + height);
  ctx.closePath();
  ctx.fill();
  ...
  ctx.beginPath();
  ctx.arc(ship.x + 2, y + height + 2, 2.5, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = blink ? '#00ff80' : '#009940';
  ctx.beginPath();
  ctx.arc(ship.x + width - 2, y + height + 2, 2.5, 0, Math.PI * 2);
  ctx.fill();
}
