...
export function handleCollisions() {
  ...
  if (ev1) { playBounce(); G.combo = 0; G.intensityDirector.onComboReset(); }
  ...
  if (ev2) {
    ...
    if (ev2.type === 'asteroidHit' || ev2.type === 'asteroidFragment') {
      ...
      G.intensityDirector.onAsteroidDestroyed(G.field.remaining, G.totalAsteroids);
  ...
  if (ev3 && ev3.type === 'gameOver') { playGameOver(); G.intensityDirector.disable(); fadeOutMusic(0.8, () => playGameOverStinger()); }
  if (ev3 && ev3.type === 'loseLife') { playLoseLife(); G.combo = 0; G.intensityDirector.onComboReset(); G.intensityDirector.onLifeChanged(ev3.livesLeft); }

  // Retarder le win pendant le slow-mo pour qu'il soit visible
  if (G.slowMoTimer <= 0) {
    const ev4 = G.session.checkWin(G.field);
    if (ev4) { playWin(); G.intensityDirector.disable(); fadeOutMusic(0.8, () => playWinStinger()); }
  }
}
